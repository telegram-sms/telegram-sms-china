...
class ussd_request_callback extends TelephonyManager.UssdResponseCallback {
    ...
    public ussd_request_callback(Context context, @NotNull SharedPreferences sharedPreferences, long message_id) {
        ...
        this.request_uri = network_func.get_url(sharedPreferences.getString("api_address", ""), bot_token, "SendMessage");
        if (message_id != -1) {
            this.request_uri = network_func.get_url(sharedPreferences.getString("api_address", ""), bot_token, "editMessageText");
